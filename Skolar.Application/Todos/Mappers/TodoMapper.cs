...
public sealed class TodoMapper : IRegister
{
    public void Register(TypeAdapterConfig config)
    {
        ...
        config.ForType<string, TodoTitle>()
              .MapWith(src => TodoTitle.Create(src).Value!);

        config.ForType<string?, TodoDescription?>()
              .MapWith(src => src == null ? null : new TodoDescription(src));

        // === Request → Command ===
        config.NewConfig<CreateTodoRequest, CreateTodoCommand>()
            .Map(dest => dest.Title, src => TodoTitle.Create(src.Title).Value!)
